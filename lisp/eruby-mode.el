...
(defface eruby-comment-face
  '((t (:inherit font-lock-comment-face :background "dim gray")))
  "Face used to highlight eRuby template snippets"
  :group 'eruby-mode-faces)
...
;; [[:ascii:][:nonascii:]] matches \n, while . does not
(defvar eruby-mode-font-lock-keywords
  '(("<%#[[:ascii:][:nonascii:]]*?%>" . '(0 'eruby-comment-face t))
    ("^\\s-*%#.*$"   . '(0 'eruby-comment-face t))
    ("<%[^#][[:ascii:][:nonascii:]]*?%>" . '(0 'eruby-standard-face t))
    ("^\\s-*%\\([^#].*\\)?$" . '(0 'eruby-standard-face t))))
